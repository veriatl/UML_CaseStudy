...
public class OCLProjector {
  ...
  protected static boolean _proj(final OperationCallExp e) {
    ...
    boolean r = false;
    final boolean src = OCLProjector.proj(e.getOwnedSource());
    final Function1<OCLExpression, Boolean> _function = (OCLExpression arg) -> {
      return Boolean.valueOf(OCLProjector.proj(arg));
    };
    final List<Boolean> args = ListExtensions.<OCLExpression, Boolean>map(e.getOwnedArguments(), _function);
    ...
    if (((!Objects.equal(args, null)) && (args.size() > 0))) {
      ...
      args_res = (IterableExtensions.<Boolean>reduce(args, _function_1)).booleanValue();
    }
  ...
  protected static boolean _proj(final PropertyCallExp e) {
    return OCLProjector.proj(e.getOwnedSource());
  }
  ...
  protected static boolean _proj(final IteratorExp e) {
    ...
    if (_equals) {
      ...
      _xifexpression = e.getReferredIteration().getName();
    }
    final String op = _xifexpression;
    boolean r = true;
    final boolean src = OCLProjector.proj(e.getOwnedSource());
    final boolean body = OCLProjector.proj(e.getOwnedBody());
