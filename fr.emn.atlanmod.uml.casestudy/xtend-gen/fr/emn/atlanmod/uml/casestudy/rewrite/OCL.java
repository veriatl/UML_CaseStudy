...
public class OCL {
  ...
  protected static String _gen(final OperationCallExp e) {
    ...
    final String src = OCL.gen(e.getOwnedSource());
    _builder.newLineIfNotEmpty();
    ...
    final String args_dot = _xifexpression_2;
    {
      if ((Objects.equal(op, "not") || Objects.equal(op, "abs"))) {
        _builder.append(op);
        _builder.append("(");
        String _gen = OCL.gen(e.getOwnedSource());
  ...
  protected static String _gen(final VariableExp e) {
    StringConcatenation _builder = new StringConcatenation();
    final String itName = OCL.genIteratorName(OCL.gen(e.getType()));
    _builder.newLineIfNotEmpty();
    final VariableDeclaration ref = e.getReferredVariable();
    {
      boolean _contains = OCL.bvMap.values().contains(ref);
      if (_contains) {
        _builder.newLineIfNotEmpty();
        ...
        {
          boolean _contains_1 = OCL.bvMap.keySet().contains(itName);
          if (_contains_1) {
  ...
  protected static String _gen(final IteratorVariable e) {
    StringConcatenation _builder = new StringConcatenation();
    final String clazz = OCL.genIteratorName(OCL.gen(e.getType()));
    _builder.newLineIfNotEmpty();
    ...
    final String hashName = ((clazz + itName) + Integer.valueOf(_hashCode));
    {
      if ((OCL.bvMap.keySet().contains(itName) && Objects.equal(OCL.bvMap.get(itName), e))) {
        ...
        if ((OCL.bvMap.keySet().contains(itName) && (!Objects.equal(OCL.bvMap.get(itName), e)))) {
          {
            boolean _contains = OCL.bvMap.keySet().contains(hashName);
            if (_contains) {
              _builder.newLineIfNotEmpty();
  ...
  protected static String _gen(final EnumLiteralExp e) {
    StringConcatenation _builder = new StringConcatenation();
    final String n = e.getReferredLiteral().getName();
    _builder.newLineIfNotEmpty();
    String _xifexpression = null;
  ...
  protected static String _gen(final TypeExp e) {
    ...
    String _xifexpression = null;
    if (OCL2ATL.postMode) {
      _xifexpression = e.getReferredType().toString().replace("::", "!").replace((OCL2ATL.model + "!"), (OCL2ATL.modelReplacer + "!"));
    } else {
      _xifexpression = e.getReferredType().toString().replace("::", "!");
    }
    _builder.append(_xifexpression);
    return _builder.toString();
  }
  ...
  protected static String _gen(final LetExp e) {
    ...
    String _xifexpression = null;
    if (OCL2ATL.postMode) {
      _xifexpression = e.getOwnedVariable().getType().toString().replace("::", "!").replace((OCL2ATL.model + "!"), (OCL2ATL.modelReplacer + "!"));
    } else {
      _xifexpression = e.getOwnedVariable().getType().toString().replace("::", "!");
    }
    _builder.append(_xifexpression);
    _builder.append(" = ");
    _builder.newLineIfNotEmpty();
    _builder.append("  ");
    String _gen = OCL.gen(e.getOwnedVariable().getOwnedInit());
    _builder.append(_gen, "  ");
    _builder.append(" in ");
    _builder.newLineIfNotEmpty();
    _builder.append("    ");
    String _gen_1 = OCL.gen(e.getOwnedIn());
  ...
  protected static String _gen(final IteratorExp e) {
    ...
    _builder.newLineIfNotEmpty();
    final HashSet<PropertyCallExp> wdExprs = OCLWDGenerator.wd(e.getOwnedBody());
    _builder.newLineIfNotEmpty();
    String _gen = OCL.gen(e.getOwnedSource());
    _builder.append(_gen);
    _builder.append("->");
                  ...
                  if (_not) {
                    ...
                    _builder.append(_xblockexpression_1);
                    {
                      ...
                      if (_not_1) {
                        ...
                        String _xifexpression_2 = null;
                        if (OCL2ATL.postMode) {
                          _xifexpression_2 = expr.getType().toString().replace("::", "!").replace((OCL2ATL.model + "!"), (OCL2ATL.modelReplacer + "!"));
                        } else {
                          _xifexpression_2 = expr.getType().toString().replace("::", "!");
                        }
  ...
  protected static String _gen(final IfExp e) {
    StringConcatenation _builder = new StringConcatenation();
    _builder.append("if (");
    String _gen = OCL.gen(e.getOwnedCondition());
    _builder.append(_gen);
    _builder.append(") then ");
    _builder.newLineIfNotEmpty();
    _builder.append("  ");
    String _gen_1 = OCL.gen(e.getOwnedThen());
    _builder.append(_gen_1, "  ");
    _builder.newLineIfNotEmpty();
    _builder.append("else ");
    _builder.newLine();
    _builder.append("  ");
    String _gen_2 = OCL.gen(e.getOwnedElse());
    _builder.append(_gen_2, "  ");
    _builder.newLineIfNotEmpty();
  ...
  public static boolean isConsistentVariable(final HashMap<String, EObject> map, final EObject exp) {
    if ((exp instanceof VariableExp)) {
      final String itName = OCL.genIteratorName(OCL.gen(((VariableExp)exp).getType()));
      return ((!map.containsKey(itName)) || Objects.equal(map.get(itName), exp));
    } else {
      if ((exp instanceof IteratorVariable)) {
        final String itName_1 = OCL.genIteratorName(OCL.gen(((IteratorVariable)exp).getType()));
  ...
  public static boolean isPrimtive(final PropertyCallExp e) {
    final String s = OCL.gen(e.getType());
  ...
  public static String genIteratorName(final String clazz) {
    ...
    for (int i = 0; (i < clazz.length()); i++) {
      boolean _isUpperCase = Character.isUpperCase(clazz.charAt(i));
      if (_isUpperCase) {
        String _rtn = rtn;
        char _lowerCase = Character.toLowerCase(clazz.charAt(i));
  ...
  public static boolean printAtHere(final PropertyCallExp e, final String v) {
    ...
    if ((_ownedSource instanceof VariableExp)) {
      String _trim = OCL.gen(e.getOwnedSource()).trim();
      ...
      if ((_ownedSource_1 instanceof PropertyCallExp)) {
        OCLExpression _ownedSource_2 = e.getOwnedSource();
        r = OCL.printAtHere(((PropertyCallExp) _ownedSource_2), v);
      }
    }
    return r;
  }
